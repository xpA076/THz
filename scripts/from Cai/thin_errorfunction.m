function delta = thin_errorfunction( r,par)  
...
c = 2.99796e8;             %光速
n = r(1)-1i*r(2);
T = (n*n_ref*4/(n+n_ref)^2*exp(-1i*(n-n_ref)*2*pi*freq*L/c))*(1-(((n-1)/(n+1))^2*exp(-2i*n*2*pi*freq*L/c))^20000)/(1-((n-1)/(n+1))^2*exp(-2i*n*2*pi*freq*L/c));   %理论计算的光学厚样品透射函数
